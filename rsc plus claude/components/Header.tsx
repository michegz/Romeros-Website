...
const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed w-full z-50 border-b transition-all duration-300 h-30 ${scrolled
      ? 'bg-stone-50/95 backdrop-blur-md py-6 border-stone-200 shadow-sm'
        ...
        <nav className="hidden lg:flex items-center gap-8 ml-14">
          ...
          <div className="flex items-center gap-4 pl-6 border-l border-stone-200">
            ...
            <a
              ...
              className="flex items-center gap-2 text-emerald-900 hover:text-emerald-700 transition-colors py-2"
            >
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden fixed inset-0 top-[60px] bg-stone-50 transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'
        ...
        <div className="p-6 flex flex-col space-y-4">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`text-xl font-bold py-4 border-b border-stone-100 ${location.pathname === link.path ? 'text-emerald-900' : 'text-stone-700'
                }`}
            >
              {link.name}
            </Link>
